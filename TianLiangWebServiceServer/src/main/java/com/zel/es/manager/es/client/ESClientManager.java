package com.zel.es.manager.es.client;
...
import com.zel.es.utils.ESCommonOperatorUtil;
...
public class ESClientManager {
	...
	public ESClientManager() {
		...
		for (ESIndexServerConfigPojo esIndexServerConfigPojo : SystemParas.esIndexServerConfigPojoList) {
			...
			// init es jest client
			for (int i = 0; i < SystemParas.jest_client_pool_size; i++) {
				esJestClientPojo = new ESJestClientPojo();
				esJestClientPojo
						.setEsIndexServerConfigPojo(esIndexServerConfigPojo);
		...
		ESCommonOperatorUtil esCommonOperatorUtil = new ESCommonOperatorUtil(
				SystemParas.es_index_server_ip,
				SystemParas.es_index_server_data_port,
				SystemParas.es_index_server_admin_port);
		ESIndexServerConfigPojo esIndexServerConfigPojo = new ESIndexServerConfigPojo(
				SystemParas.es_index_server_ip,
				SystemParas.es_index_server_data_port,
				SystemParas.es_index_server_admin_port);
		// init es self client for port admin port
		for (int i = 0; i < SystemParas.es_client_pool_size; i++) {
